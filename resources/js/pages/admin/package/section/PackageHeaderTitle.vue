    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6 dark:border-zinc-800">
        <div>
            <div class="flex flex-wrap items-center gap-2 mb-2">
                <span v-if="package.is_featured" class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-400">
                    <Star class="w-3 h-3 fill-current" /> Featured
                </span>
                <span v-if="package.is_foreign_only" class="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">
                    Foreigners Only
                </span>
                <span :class="['text-xs font-semibold px-2.5 py-0.5 rounded-full capitalize', getSeasonColor(package.season)]">
                    {{ package.season }}
                </span>
                <span v-if="package.tag" class="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ package.tag }}
                </span>
            </div>
