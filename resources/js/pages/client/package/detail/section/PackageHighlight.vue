...
<template>
  <section class="w-full h-auto relative overflow-hidden p-4">
    <div class="max-w-5xl m-auto w-full flex flex-col justify-start gap-2 p-4 border"
      ...
      <div class="w-full flex flex-col gap-1">
        <transition-group name="list" tag="div" class="flex flex-col gap-1">
          <span v-for="(data, index) in visibleHighlights" :key="index"
            class="flex items-start gap-2 text-sm md:text-base">
            <Icon icon="material-symbols:check" class="text-xl md:text-2xl shrink-0"
