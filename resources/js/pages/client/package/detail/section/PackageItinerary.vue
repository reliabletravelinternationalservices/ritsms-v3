...
<template>
  <section class="w-full h-auto relative overflow-hidden p-2 sm:p-4">
    <div class="max-w-5xl mx-auto w-full flex flex-col justify-start gap-3 p-2 sm:p-4">
        <h2 class="font-bold font-roboto text-sm md:text-lg uppercase tracking-wide">Itineraries</h2>
        ...
        <Accordion type="single" collapsible class="w-full" default-value="day-1">
            ...
            >
                ...
                <AccordionContent>
                <ul class="flex flex-col gap-2.5 py-2 px-1 sm:px-6">
                    <li 
                      v-for="(activity, aIndex) in item.activity" 
                      :key="aIndex" 
                      class="flex items-start gap-2 min-w-0"
                    >
                        <div class="mt-1.5 sm:mt-2 w-1.5 h-1.5 rounded-full shrink-0" :class="cn(isInbound ? 'bg-[var(--inbound-custom)]' : 'bg-[var(--outbound-custom)]')" aria-hidden="true"></div>
