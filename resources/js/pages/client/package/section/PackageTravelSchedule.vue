...
<template>
  <section v-if="props.package.schedules && props.package.schedules.length > 0" class="w-full h-auto relative overflow-hidden p-4">
        <div class="max-w-5xl m-auto w-full flex flex-col justify-start gap-1 p-4" :class="cn(isInbound? 'bg-[var(--inbound-opacity-custom-10)]' : 'bg-[var(--outbound-opacity-custom-10)]')">
