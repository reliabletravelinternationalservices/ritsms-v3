...
<template>
    <section class="w-full relative flex" aria-label="Filter travel packages">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full">
                ...
                <div class="space-y-2">
                    <span class="flex items-end text-[var(--secondary-custom)]">
                        <Icon icon="lucide:map-pinned" width="20" height="20" aria-hidden="true" />
                        <label for="filter-destination" class="ml-2 text-sm font-bold uppercase tracking-wider">Location</label>
                    </span>
                    <DropdownField id="filter-destination" v-model="form.destination" placeholder="All Location"
                        :options="props.destinationOptions" />
                </div>

                <div class="space-y-2">
                    <span class="flex items-end text-[var(--secondary-custom)]">
                        <Icon icon="lucide:calendar" width="20" height="20" aria-hidden="true" />
                        <label for="filter-season" class="ml-2 text-sm font-bold uppercase tracking-wider">Season</label>
                    </span>
                    <DropdownField id="filter-season" v-model="form.season" placeholder="All Season" :options="props.seasonOptions" />
                </div>

                <div class="space-y-2 sm:col-span-2 md:col-span-1">
                    <span class="flex items-end text-[var(--secondary-custom)]">
                        <Icon icon="lucide:clock-4" width="20" height="20" aria-hidden="true" />
                        <label for="filter-duration" class="ml-2 text-sm font-bold uppercase tracking-wider">Duration</label>
                    </span>
                    <DropdownField id="filter-duration" v-model="form.duration" placeholder="All Duration"
                        :options="props.durationOptions" />
                </div>
            </div>

            <div class="w-full md:w-auto flex flex-col sm:flex-row gap-3">
                <button type="button" @click.prevent="searchPackages"
                    ...
                    <Icon icon="material-symbols:search" width="24" height="24" aria-hidden="true" />
                </button>
