...
<template>
            <Link :href="isInbound ? route('client.inbound.package.detail', { package: package.id }) : route('client.outbound.package.detail', { package: package.id })" class="carousel__item">
            <div class=" h-auto md:h-[400px] max-w-[320px] flex flex-col md:grid md:grid-rows-2 border border-[var(--shadow-custom)] overflow-hidden bg-white">
              ...
              <div class="relative h-[180px] md:h-full">
                <img 
                  v-if="package.primary_image"
                  :src="package.primary_image.url" 
                  :alt="package.name"
                ...
                <span v-if="package.tag != null" class="bg-black/50 flex items-center justify-center absolute top-0 left-0 py-1 px-4 text-[var(--primary-custom)] font-roboto m-2 text-[10px] md:text-xs">
                  {{ package.tag }}
                </span>
              </div>
                  ...
                  <div>
                      ...
                      <div class="flex flex-col">
                          ...
                          <h4 class="font-medium font-roboto text-[10px] md:text-sm text-[var(--muted-custom)] mb-4">
                            from <span class="font-bold text-[var(--warning-custom)] text-sm md:text-lg">{{ formatCurrency(usdRate? (package.base_price/usdRate) : package.base_price, isInbound? 'USD' : 'PHP') }}/pax</span>
                          </h4>
