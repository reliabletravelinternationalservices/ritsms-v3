...
<template>
    <div class="space-y-12">
        ...
        <div class="space-y-4">
            ...
            <!-- NO ITINERARY TYPE -->
            <div v-if="!tourForm.containsRoute()"
            ...
            <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
                ...
                <div v-for="(route, index) in tourForm.form.routes" :key="index"
                    class="space-y-6 rounded-md border border-border p-8">

                    <!-- ROUTE HEADER -->
                    <div class="flex items-center justify-between text-sm font-bold text-muted-foreground uppercase">
                        ...
                        <ButtonIcon :disabled="isLoading" @click="tourForm.removeRoute(index)" icon="lucide:trash-2"
                            class="text-red-600 bg-transparent hover:bg-red-50" />
                    </div>

                    <!-- DEPARTURE / DESTINATION -->
                    <div class="flex flex-col md:flex-row md:items-start gap-4 md:gap-6 w-full">
                        ...
                        <div class="w-full space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <SelectMenu :disabled="isLoading" :id="`departure-route-${index + 1}-country`"
                                    v-model="route.departure_country_id" placeholder="Select country"
                                    :options="referenceData.countryOptions" class="font-roboto text-sm" />
                                <InputError :message="tourForm.errors['routes.' + index + '.departure_country_id']" />
                            </div>

                            <div class="space-y-2">
                                <label :for="`departure-route-${index + 1}-location`" class="block text-sm font-medium">
                                    Departure Location
                                    <span class="text-red-600">*</span>
                                </label>

                                <Input :disabled="isLoading" :id="`departure-route-${index + 1}-location`"
                                    v-model="route.departure_city" placeholder="Location" class="font-roboto text-sm" />
                        ...
                        <!-- ARROW -->
                        <span class="self-center text-sm font-bold text-muted-foreground uppercase">
                        ...
                        <div class="w-full space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <SelectMenu :disabled="isLoading" :id="`destination-route-${index + 1}-country`"
                            ...
                            <div class="space-y-2">
                                <label :for="`destination-route-${index + 1}-location`"
                                    class="block text-sm font-medium">
                                    Destination Location
                                    <span class="text-red-600">*</span>
                                </label>

                                <Input :disabled="isLoading" :id="`destination-route-${index + 1}-location`"
                                    v-model="route.destination_city" placeholder="Location"
            ...
            <div v-if="!tourForm.isOnRouteLimit()" class="w-full">
                <button :disabled="isLoading" type="button" @click="tourForm.addRoute()"
