        ...
        <div class="space-y-4">
            ...
            <div v-if="!tourForm.containsItineraryType()"
                ...
                <div class="mb-2 rounded-full bg-muted p-3">
                    <Icon icon="lucide:airplane" class="size-5 text-muted-foreground" />
                </div>
            ...
            <div class="w-full">
                <button v-if="tourForm.containsMultipleFlights()" type="button" @click="tourForm.addRoute()"
                    class="w-full text-foreground bg-[rgb(var(--color-primary)/0.08)] border border-dashed border-border px-4 py-2.5 text-sm font-medium transition-all duration-150 rounded-md hover:bg-[rgb(var(--color-primary)/0.1)]">+
                    Add Another
                    Route</button>
            </div>
        </div>
