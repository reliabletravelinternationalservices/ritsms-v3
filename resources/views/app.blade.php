...
<head>
    ...
    @if (!empty($seo) && $seo != null)
        @php
            $seoData = $seo ?? [];
            $seoTitle = $seoData['title'] ?? 'Reliable International Travel Services';
            $seoDescription =
                $seoData['description'] ??
                'Reliable International Travel Services offers expertly curated inbound and outbound travel packages, destination guides, and personalized travel planning.';
        ...
        <meta head-key="twitter:description" name="twitter:description"
            content="{{ Str::limit($seoDescription, 160) }}" />
